...
export default function SignupPage() {
  ...
  if (registeredSuccess) {
    return (
      <div className="min-h-screen bg-[var(--bg-main)] flex items-center justify-center p-4 md:p-8 select-none transition-colors">
        <div className="w-full max-w-md bg-[var(--bg-card)] border border-[var(--border-color)] rounded-3xl shadow-2xl p-8 text-center space-y-6 text-[var(--text-primary)]">
          <div className="mb-2">
          ...
          <div className="space-y-2">
            <h2 className="text-2xl font-extrabold text-[var(--text-primary)] tracking-tight">Account Created Successfully</h2>
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-main)] flex items-center justify-center p-4 md:p-8 select-none transition-colors">
      <div className="w-full max-w-5xl bg-[var(--bg-card)] border border-[var(--border-color)] rounded-3xl shadow-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-2 text-[var(--text-primary)]">
        {/* Left Form Panel */}
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              <div>
                ...
                <div className="relative">
                  <Briefcase className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-[var(--text-muted)]" />
                  <select
                    value={role}
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setRole(e.target.value as 'TEAM_MEMBER' | 'PROJECT_MANAGER')}
                    className="w-full bg-[var(--bg-input)] border border-[var(--border-color)] rounded-xl pl-10 pr-3 py-2.5 text-sm text-[var(--text-primary)] focus:outline-none focus:border-[#5B82FF] transition-all cursor-pointer"
                  >
              ...
              {/* Terms Checkbox */}
              <div className="pt-1">
                <label className="flex items-start gap-2.5 text-xs text-[var(--text-secondary)] cursor-pointer hover:text-[var(--text-primary)] transition-colors">
                  <input
                    type="checkbox"
                    required
                    checked={acceptTerms}
                    onChange={(e) => setAcceptTerms(e.target.checked)}
                    className="mt-0.5 w-4 h-4 rounded border-[var(--border-color)] bg-[var(--bg-input)] text-[#4E75FF] focus:ring-0 accent-[#4E75FF]"
                  />
                  <span>
                    I agree to the{' '}
        ...
        <div className="hidden lg:flex flex-col justify-between p-12 bg-gradient-to-br from-[#4E75FF]/10 via-[var(--bg-card-hover)] to-[var(--bg-card)] border-l border-[var(--border-color)] relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-sm text-[var(--text-secondary)] mt-3 leading-relaxed">
              Orbitask offers dark & day theme dashboard execution, timeline metrics, team member task assignments, and seamless workflow management.
            </p>
          </div>

          <div className="relative z-10 space-y-3 my-6">
            <div className="bg-[var(--bg-card)]/80 border border-[var(--border-color)] rounded-xl p-3.5 flex items-center justify-between text-xs">
              <span className="text-[var(--text-primary)] font-medium">✨ Real-time task board sync</span>
              <span className="text-emerald-600 dark:text-emerald-400 font-semibold">Active</span>
            </div>
            <div className="bg-[var(--bg-card)]/80 border border-[var(--border-color)] rounded-xl p-3.5 flex items-center justify-between text-xs">
              <span className="text-[var(--text-primary)] font-medium">⚡ Instant sprint progress reports</span>
